...
export class ParticleSimulation {
  ...
  setupEventListeners() {
    ...
    if (glowIntensity && glowIntensityValue) {
  ...
  pushParticlesFromCursor() {
    ...
    this.particles.forEach((particle) => {
      ...
      if (dist < this.config.pushRadius && dist > 0.1) {
        ...
        const ny = dy / dist;
        const force =
          (this.config.pushStrength * (1 - dist / this.config.pushRadius)) /
          particle.size;
  ...
  updateStats() {
    ...
    if (now >= this.stats.lastFrameTime + 1000) {
      this.stats.fps = Math.round(
        (this.stats.frameCount * 1000) / (now - this.stats.lastFrameTime)
      );
...
drawParticleGlow(particle) {
    ...
    const color = this.config.glowColor || "#ffffff";
